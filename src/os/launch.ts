...
import { detectBrowserPlatform } from '../utils/detect-platform';
...
function subscribeToProcessEvent(event: 'exit' | 'SIGINT' | 'SIGHUP' | 'SIGTERM', handler: EventHandler): void {
...
function unsubscribeFromProcessEvent(event: 'exit' | 'SIGINT' | 'SIGHUP' | 'SIGTERM', handler: EventHandler): void {
  const listeners = processListeners.get(event) || [];
...
export class Process {
  ...
  constructor(opts: LaunchOptions) {
    ...
    debugLaunch(`Launching ${this.#executablePath} ${this.#args.join(' ')}`, {
      ...
      env: Object.keys(env).reduce<Record<string, string | undefined>>((res, key) => {
  ...
  #configureStdio(opts: { pipe: boolean; dumpio: boolean }): Array<'ignore' | 'pipe'> {
    if (opts.pipe) {
      if (opts.dumpio) {
  ...
  waitForLineOutput(regex: RegExp, timeout = 0): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      function onTimeout(): void {
        cleanup();
        reject(
          new TimeoutError(`Timed out after ${timeout} ms while waiting for the WS endpoint URL to appear in stdout!`),
        );
      }
